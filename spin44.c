...
int main()
{
    ...
    for (i=0;i<40;i++)// give 0 to all the elements in the lattice..
    {
        for(j=0;j<40;j++)
        {
            lts[i][j]=1;
    ...
    srand48(time(NULL));
    for(Ns=1;Ns<=505000;Ns++)// sweep for 5000 times and then begin measurements..
    {
        for (i=0;i<40;i++)
        {
            for(j=0;j<40;j++)
        ...
        if(Ns > 5000 && Ns%500 == 0) // make measurements and record in array..
        {
            M[ii] = avg(lts) / 1600.0;
